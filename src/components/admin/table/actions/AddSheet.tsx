...
export const AddSheet = ({
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_URL}/${route_name}/create`, {
                method: 'POST',
    ...
    const removeListItem = (fieldName: string, index: number) => {
        setFormData((prev) => ({
            ...prev,
            [fieldName]: (prev[fieldName] as string[]).filter(
                (_, i) => i !== index
            ),
        }))
    }
        ...
        <Sheet open={isAdding} onOpenChange={setIsAdding}>
            ...
            <SheetContent className="overflow-y-auto max-h-screen px-2">
                <SheetHeader>
                    ...
                    <SheetDescription>
                        Fill in the details for the new {route_name}. Upload an
                        image and add translations.
                    </SheetDescription>
                </SheetHeader>
                        ...
                        field.type === 'file-image' ? (
                            <div className="grid gap-2" key={field.name}>
                                ...
                                <input
                                    ...
                                    accept="image/*"
                                    onChange={(e) =>
                                        handleImageUpload(e, field.name)
                                    }
                                    className="hidden"
                            ...
                            <div key={field.name} className="space-y-3">
                                ...
                                {(['uz', 'oz', 'ru', 'en'] as const).map(
                                    (lang) => {
                                        const localized = formData[
                                            field.name
