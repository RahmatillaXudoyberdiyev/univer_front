...
export const EditSheet = ({ id, route_name, fields }: EditSheetProps) => {
  ...
  useEffect(() => {
    if (isEditing && !data) {
      const fetchData = async () => {
        try {
          const res = await fetch(`${baseBackendUrl}/${route_name}/${id}`)
  ...
  const handleEdit = async () => {
    ...
    if (!data) return
    try {
      const res = await fetch(`${baseBackendUrl}/${route_name}/update/${id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleFileUpload = async (
    ...
    try {
      const res = await fetch(`${baseBackendUrl}/upload`, {
        method: 'POST',
        body: formData,
      })
  ...
  const handleListChange = (fieldKey: string, index: number, value: string) => {
    setData((prev: any) => {
          ...
          {loading ? (
            ...
            <div className="grid gap-6 pt-4 px-2">
              {fields.map((field) => {
                ...
                if (field.type === 'file-image') {
                  return (
                    <div className="grid gap-2" key={field.name}>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          document
                            .getElementById(`edit-image-upload-${field.name}`)
                            ?.click()
                        }
                      >
                  ...
                  return (
                    <div className="grid gap-2" key={field.name}>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          document
                            .getElementById(`edit-file-upload-${field.name}`)
                            ?.click()
                        }
                      >
                    ...
                    <div className="grid gap-2" key={field.name}>
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setData((prev: any) => ({
                            ...prev!,
                            [field.name]: val ? new Date(val) : null,
                          }))
                        }}
                      />
                    </div>
                    ...
                    <div key={field.name} className="space-y-3">
                      ...
                      {(['uz', 'oz', 'ru', 'en'] as const).map((lang) => (
                        <div key={lang} className="flex items-start gap-5">
                          ...
                          <Input
                            value={(data[field.name]?.[lang] as string) ?? ''}
                            onChange={(e) =>
                              setData((prev: any) => ({
                                ...prev!,
                                [field.name]: {
                ...
                } else if (field.type === 'localized-markdown') {
                  return (
                  ...
                  return (
                    <div key={field.name} className="space-y-4">
                      ...
                      {(['uz', 'oz', 'ru', 'en'] as const).map((lang) => {
                        const list =
                          (data[field.name]?.[lang] as string[]) || []
                        return (
                          <div key={lang} className="border rounded-md p-3">
                            <div className="text-sm font-medium text-muted-foreground mb-2">
                              {languageNames[lang]}
