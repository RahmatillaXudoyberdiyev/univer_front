...
const Header = () => {
  ...
  const menuItems = [
    ...
    ...(menus.data?.map((menu: any) => ({
      title: menu.name?.[locale] || menu.name?.uz || 'Noma’lum', // fallback bilan
      items: {
        default: `/${menu.slug}`, // asosiy menu sahifasi (agar kerak bo'lsa)
        // Submenularni to'g'ri object ga aylantiramiz
        ...(menu.submenus?.reduce(
          (acc: Record<string, string>, submenu: any) => {
            const submenuTitle =
              submenu.name?.[locale] || submenu.name?.uz || 'Noma’lum'
            acc[submenuTitle] = `/${menu.slug}/${submenu.slug}` // yoki faqat /submenu.slug
            return acc
          },
          {}
        ) || {}),
